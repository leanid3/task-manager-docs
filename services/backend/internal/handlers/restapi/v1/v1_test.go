...
type TaskLLMUCMock struct {
	mock.Mock
}

func (m *TaskLLMUCMock) CreateTask(ctx context.Context, r io.Reader, filename string, size int64, requestID string) (uuid.UUID, error) {
	args := m.Called(ctx, r, filename, size, requestID)
	return args.Get(0).(uuid.UUID), args.Error(1)
}

func (m *TaskLLMUCMock) GetTaskByID(ctx context.Context, id uuid.UUID) (*domain.Task, error) {
...
func (m *TaskLLMUCMock) UpdateTaskStatus(ctx context.Context, evt domain.TaskLLMStatusEvent) error {
...
func TestCreateTask(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			v1Handler := newTestV1()
			taskMock := v1Handler.uc.TaskLLMUC.(*TaskLLMUCMock)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.mockTask != nil || tt.mockError != nil {
				validUUID := uuid.MustParse(tt.taskIDStr) // НЕ падает на "invalid-uuid"
