...
func TestTaskRepositoryUpdateStatusCompletedSetsTimestamp(t *testing.T) {
	...
	taskID := uuid.New()
	_, err := pool.Exec(ctx, `
        INSERT INTO tasks (task_id, status, storage_path, storage_size, created_at)
        VALUES ($1, $2, $3, $4, $5)
    `, taskID, "PENDING", "/bucket/test.pdf", 1024, time.Now())
	require.NoError(t, err)
...
func TestTaskRepositoryListByStatus(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, err := pool.Exec(ctx, `
            INSERT INTO tasks (task_id, status, storage_path, storage_size, created_at)
            VALUES ($1, $2, $3, $4, $5)
        `, uuid.New(), "PENDING", "/bucket/test.pdf", 1024, now.Add(time.Duration(i)*time.Second))
		require.NoError(t, err)
	}
	...
	for i := 0; i < 2; i++ {
		_, err := pool.Exec(ctx, `
            INSERT INTO tasks (task_id, status, storage_path, storage_size, created_at)
            VALUES ($1, $2, $3, $4, $5)
        `, uuid.New(), "COMPLETED", "/bucket/test.pdf", 1024, now)
		require.NoError(t, err)
	}
...
func TestTaskRepositoryListByStatusLimit(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		_, err := pool.Exec(ctx, `
            INSERT INTO tasks (task_id, status, storage_path, storage_size, created_at)
            VALUES ($1, $2, $3, $4, $5)
        `, uuid.New(), "PENDING", "/bucket/test.pdf", 1024, time.Now())
		require.NoError(t, err)
	}
...
func TestTaskRepositoryCreateJSONMetadata(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		INSERT INTO tasks (task_id, status, metadata, created_at)
		VALUES ($1, $2, $3, $4)
	`, taskID, "PENDING", metadataJSON, time.Now())
	require.NoError(t, err)
