...
func TestTaskUCCreateTask(t *testing.T) {
	tests := []struct {
		...
		filesize      int64
		requestID     string
		setupMocks    func(*mocks.MockRepository, *mocks.MockProducer, *mocks.MockStorageRepository, *mocks.MockLogger)
		expectedError bool
	...
	}{
		{
			...
			filesize:  100,
			requestID: "req-123",
			setupMocks: func(repo *mocks.MockRepository, producer *mocks.MockProducer, storage *mocks.MockStorageRepository, logger *mocks.MockLogger) {
				// Мокируем BucketName
				...
				producer.On("Send", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
					Return(nil)
				//Мокируем логирование - принимаем любое количество аргументов
				// Info вызывается с msg (string) + переменное количество пар ключ-значение
				// Используем mock.Anything для всех возможных аргументов (до 13)
				logger.On("Info", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
			},
			expectedError: false,
		},
		{
			...
			filesize:  100,
			requestID: "req-123",
			setupMocks: func(repo *mocks.MockRepository, producer *mocks.MockProducer, storage *mocks.MockStorageRepository, log *mocks.MockLogger) {
			},
			expectedError: true,
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockRepository, producer *mocks.MockProducer, storage *mocks.MockStorageRepository, log *mocks.MockLogger) {
				...
				log.On("Error", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
			},
			expectedError: true,
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockRepository, producer *mocks.MockProducer, storage *mocks.MockStorageRepository, logger *mocks.MockLogger) {
				...
				logger.On("Info", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
				logger.On("Error", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
			},
			expectedError: true,
		...
		},
		{
			...
			setupMocks: func(repo *mocks.MockRepository, producer *mocks.MockProducer, storage *mocks.MockStorageRepository, logger *mocks.MockLogger) {
				...
				repo.On("Create", mock.Anything, mock.Anything).
					Return(nil)
				repo.On("UpdateWithError", mock.Anything, mock.Anything, mock.Anything).
					Return(nil)
				producer.On("Send", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
...
func TestTaskUCGetTaskByID(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			taskID: uuid.New(),
			setupMocks: func(repo *mocks.MockRepository, logger *mocks.MockLogger) {
				repo.On("GetByID", mock.Anything, mock.Anything).
					Return(&domain.Task{
						TaskID: uuid.New(),
					}, nil)
				logger.On("Debug", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
			},
			expectedError: false,
		},
		{
